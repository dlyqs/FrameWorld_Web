...
const clearMessages = () => {
  // 假设 props.conversation.messages 是绑定到消息列表的
  // 这里直接将其设置为空数组即可清空消息显示
  props.conversation.messages = [];
};

// 添加计算属性来获取最后一条机器的消息
const lastBotMessage = computed(() => {
  // 假设每条消息都有一个is_bot标识和created_at时间戳
  // 过滤出机器的消息，并根据created_at进行排序，取最新的一条
      ...
      async onmessage(message) {
        ...
        if (event === 'done') {
          ...
          if (!props.conversation.id) {
            ...
            genTitle(props.conversation.id,props.sourcePage)
          }
          if (data.newDocId) {
...
defineExpose({ applyPrompt, triggerSend, clearMessages });
